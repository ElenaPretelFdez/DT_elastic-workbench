...
def map_to_index(value, space):
    return int(np.argmin(np.abs(space - value)))
...
def train_pymdp_agent(action_selection, alpha, motivate_cores):
    ...
    for steps in range(50):
        ...
        # Inferencia de estados actuales más probables
        ...
        #print("pymdp_state: ", pymdp_state)
        #print("(next_state_qr, next_state_cv): ", (next_state_qr, next_state_cv))


        pymdp_state = [
            np.int64(map_to_index(next_state_cv.throughput, p_agent.throughput_cv)),        # cv_throughput
            np.int64(map_to_index(next_state_cv.data_quality, p_agent.quality_cv)),         # cv_data_quality
            next_state_cv.model_size,                                                       # cv_model_size
            next_state_cv.cores,                                                            # cv_cores
            np.int64(map_to_index(next_state_qr.throughput, p_agent.throughput_qr)),        # qr_throughput
            np.int64(map_to_index(next_state_qr.data_quality, p_agent.quality_qr)),         # qr_data_quality
            next_state_qr.cores                                                             # qr_cores
        ]
        ...
        print("reward", logged_data[-1]["reward"])
